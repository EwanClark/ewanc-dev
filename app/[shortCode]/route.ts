...
import { getLocationFromIP, parseUserAgent, formatDeviceType } from '@/lib/analytics';
...
export async function GET(
  ...
  try {
    ...
    if (shortUrl.password_hash) {
      ...
      console.log(`Password protection flow - URL: ${url.toString()}`);
      console.log(`Provided password: ${providedPassword ? '[PROVIDED]' : '[NOT PROVIDED]'}`);
      ...
      if (!providedPassword) {
        ...
        console.log(`Created new click record with ID: ${newClickId}`);
        
        // Redirect to password form with click ID
        return NextResponse.redirect(
          new URL(`/password-required?shortCode=${shortCode}&clickId=${newClickId}`, request.url)
        );
      }

      const isPasswordValid = await bcrypt.compare(providedPassword, shortUrl.password_hash);
      if (!isPasswordValid) {
        // Track failed attempt if no existing click ID
        if (!clickId) {
          const newClickId = await trackClick(request, shortUrl.id, false);
          return NextResponse.redirect(
            new URL(`/password-required?shortCode=${shortCode}&error=invalid&clickId=${newClickId}`, request.url)
          );
        } else {
          return NextResponse.redirect(
            new URL(`/password-required?shortCode=${shortCode}&error=invalid&clickId=${clickId}`, request.url)
          );
        }
      }
      ...
      if (clickId) {
        console.log(`Updating click ${clickId} to authorized: true`);
        const updateSuccess = await updateClickAuthorization(clickId, true);
        if (updateSuccess) {
          console.log(`Successfully updated click ${clickId} to authorized`);
        } else {
          console.error(`Failed to update click ${clickId} to authorized`);
        }
      } else {
        // Track successful attempt if no existing click ID
        console.log('No clickId provided, creating new authorized click record');
...
async function trackClick(request: NextRequest, shortUrlId: string, authorized: boolean | null): Promise<string | null> {
  try {
    const supabase = await createClient();
    ...
    console.log('Track click result:', data);
    const clickId = data?.id || null;
    console.log('Returning click ID:', clickId);
    return clickId;
  } catch (error) {
...
async function updateClickAuthorization(clickId: string, authorized: boolean): Promise<boolean> {
  try {
    ...
    console.log(`Attempting to update click ${clickId} with authorized: ${authorized}`);
    ...
    if (!data || data.length === 0) {
      console.error('No rows were updated - click record may not exist or RLS policy blocking update');
      return false;
    }
    
    console.log('Successfully updated click record:', data[0]);
    return true;
  } catch (error) {
